...
const RectorDashboard = () => {
  ...
  // It's recommended to use environment variables for your API URL
  ...
  const fetchLeaves = async () => {
    try {
      const res = await axios.get(`${apiurl}/api/rector/all`, {
  ...
  const updateLeaveStatus = async (leaveId, status) => {
    try {
      await axios.put(`${apiurl}/api/rector/status/${leaveId}`, { status }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setLeaves((prev) =>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white p-6 sm:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="overflow-x-auto bg-slate-800 border border-slate-700 rounded-xl shadow-2xl shadow-slate-950">
          {loading ? (
            <p className="text-center text-slate-400 text-lg py-16">Loading applications...</p>
          ) : (
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {leaves.length > 0 ? (
                  leaves.map((leave) => (
                    <tr key={leave._id} className="border-b border-slate-700 last:border-b-0 hover:bg-slate-700/50 transition-colors duration-200">
...
//   return (
//     <div className="p-4">
//       <h2 className="text-2xl font-bold mb-4">Rector - Leave Applications</h2>
